...
export function analyzeSpendBySupplier(pos: PurchaseOrder[], limit: number = 20): SpendBySupplier[] {
...
export function analyzeSupplierPerformance(pos: PurchaseOrder[]): SupplierPerformance[] {
  ...
  return Array.from(supplierMap.entries())
    .map(([supplier, data]) => {
      const avgLeadTime = data.leadTimes.length > 0
        ? data.leadTimes.reduce((a, b) => a + b, 0) / data.leadTimes.length
        : 0

      const avgPrice = data.prices.reduce((a, b) => a + b, 0) / data.prices.length
      const priceVariance = data.prices.length > 1
        ? Math.sqrt(data.prices.reduce((sum, price) => sum + Math.pow(price - avgPrice, 2), 0) / data.prices.length) / avgPrice * 100
        : 0
